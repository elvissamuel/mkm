...
// import { UpdateProgramSchema } from "@/lib/validation-schema"
import { Program } from "@prisma/client"
import { UpdateProgramSchema, updateProgramSchema } from "@/lib/validation-schema"

interface EditProgramFormProps {
  program: Program & { local_price: string }
  isOpen: boolean
  onOpenChange: (open: boolean) => void
  onSubmit: (data: z.infer<typeof updateProgramSchema>) => void
}

export function EditProgramForm({ program, isOpen, onOpenChange, onSubmit }: EditProgramFormProps) {
  const form = useForm<UpdateProgramSchema>({
  ...
  const handleSubmit = (data: z.infer<typeof updateProgramSchema>) => {
    onSubmit(data)
    form.reset()
    onOpenChange(false)
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="bg-[#0e1723] border-[#c19b37]/20 text-gray-200">
        <DialogHeader>
          <DialogTitle className="text-[#c19b37]">Edit Program</DialogTitle>
